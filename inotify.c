...
int
inotify_insert_file(int update, char * name, const char * path)
{
	int len;
	char * last_dir;
	char * path_buf;
...
inotify_insert_directory(int fd, char *name, const char * path)
{
	...
	while( readdir_r(ds, d, &e) == 0 && e != NULL )
	{
		...
		else if( type == TYPE_FILE || ends_with(esc_name, ".srt") )
		{
			if( (stat(path_buf, &st) == 0) && (st.st_blocks<<9 >= st.st_size) )
			{
				inotify_insert_file(1, esc_name, path_buf);
			}
		}
		free(esc_name);
	}
...
inotify_remove_directory(int fd, const char * path)
{
	...
	sql_exec(db, "DELETE from PLAYLISTS where (PATH > '%q/' and PATH <= '%q/%c')", path, path, 0xFF);
	sql_exec(db, "DELETE from CAPTIONS where (PATH > '%q/' and PATH <= '%q/%c')", path, path, 0xFF);
