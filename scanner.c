...
insert_containers(const char * name, const char *path, const char * refID, const char * class, sqlite3_int64 detailID)
{
	...
	if( strstr(class, "imageItem") )
	{
		...
		char date_taken[128], camera[256];
		...
		if( date )
		{
			snprintf(date_taken, sizeof(date_taken), "%s", date);
		}
		else
		{
		...
		if( cam )
		{
			snprintf(camera, sizeof(camera), "%s", cam);
		}
		else
		{
...
void
ScanDirectory(const char * status_file, int media_dir_size, const char * dir, const char * parent, enum media_types dir_type)
{
	...
	for (i=0; i < n; i++)
	{
		...
		if( (type == TYPE_DIR) && (access(full_path, R_OK|X_OK) == 0) &&
			(parent != NULL || strcmp(namelist[i]->d_name, DLNA_DB_CACHE_DIR) != 0) )
		{
			/* do not scan DLNA_DB_CACHE_DIR in a parent directory */
			if( GETFLAG(UPDATE_SCAN_MASK) )
			{
				...
				if( objectID )
				{
					ScanDirectory(status_file, media_dir_size, full_path, objectID+2, dir_type);
...
void
start_scanner(const char *const status_file)
{
	...
	while( media_path )
	{
		...
		ScanDirectory(status_file, strlen(media_path->path), media_path->path, NULL, media_path->type);
	...
	sql_exec(db, "drop INDEX IDX_SEARCH_OPT");
